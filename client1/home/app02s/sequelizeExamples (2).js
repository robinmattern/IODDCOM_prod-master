...
// ---------------------------------------------------------------
async function DropDatabase_query() {
  try {
    const values = await sequelize.query("DROP DATABASE practice");
    console.log("--Database practice has been dropped");
  } catch (error) {
    console.error("--Unable to drop the practice database:", error);
  }
}
// ---------------------------------------------------------------
