...
//     aTests   = 'test2 fetch /projects the  old way, rjs format'  // iodd.com/client3/c33_home-wi-mysql-api'
//     aTests   = 'test3 fetch /projects with API_URL, rjs format'
//     aTests   = 'test4 fetch /projects the  new way, rjs format'
  ...
  var  aAPI     = `${document.location.href}`.match( /iodd.com/i ) ? '/api3' : ''               // .(30214.03.1 RAM Set if not running in Browser).(30220.02.1 RAM Add here)
       }
  var  aAPI_URL =  aAPI ? '/api1' : 'http://localhost:3003'                                     // .(30214.03.2).(30220.02.2) 
//---------------------------------------------------------------------------------------------------
...
if ( aTests.match( /test2/ ) ) { 
    ...
    fetch( 'http://localhost:3003/projects?recs=5' )
           .then( ( res  ) => res.json( ) )
...
//                      fetchJSON( 'http://localhost:3003/projects?recs=2' ).then( pJSON => {   //#.(30220.02.3) 
                        fetchJSON(           `${aAPI_URL}/projects?recs=2` ).then( pJSON => {   // .(30220.02.3)  

       var  aHTML    =  fmtProjects( pJSON.projects )                                           // .(30209.01.6)                                                   
...
if (aTests.match( /test4/ ) ) { 

                 fetchProjects()                                                                // .(30209.01.2 RAM Move fetch into this script)

            } // eof test4
//---------------------------------------------------------------------------------------------------

async function fetchProjects( pDiv ) {                                                          // .(30209.01.3 RAM Beg Write fetchProjects)

//     var  mProjects = (await fetchJSON( 'http://localhost:3003/projects?recs=999' )).projects //#.(30220.02.4)  

       var  mProjects = (await fetchJSON(           `${aAPI_URL}/projects?recs=999` )).projects // .(30220.02.4)  
       ...
       var  aHTML     =  fmtProjects( mProjects)                                                 // .(30209.01.4)

       if (pDiv) { pDiv.innerHTML =  aHTML 
       } else {    console.log( aHTML ) }

            } // eof fetchProjects1                                                              // .(30209.01.3 RAM Beg Write fetchProjects)
//--------  -------  =  -------------------------------------------------------

//nction  fmtProjects(   pJSON   ) {                                                             //#.(30209.01.5)  
function  fmtProjects( mProjects ) {                                                             // .(30209.01.5 RAM Pass mProjects, not pJSON)  
...
//     ---  --------  =  -----------------------------------
  ...
  function  fmtProject( pProject, i ) {                                                         // .(30208.02.1 RJS Beg Rewrite)
       ...
       var  aName         =  pProject.ProjectName  //#.(30214.01.1 RAM Was: pProject.ProjectName).(30220.05.1 RAM It's now ProjectName)
...
//          aData = aHeadRow + aRow
    ...
    return  aData    
 //         -------  =  ----------------------
            }   // eof  fmtProject                                                              // .(30208.02.1 RJS End) 
//     ---  -------  =  ----------------------------------
            }   // eof  fmtProjects                                                            
//--------  -------  =  -------------------------------------------------------
